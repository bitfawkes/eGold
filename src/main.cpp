...
#if defined(NDEBUG)
# error "eGold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x036128aa36257b2135a7a107afcc4b1b546b5c49ca693abe33095383059f21e3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eGold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eGold Signed Message:\n";
...
{
    // eGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 4 blocks
    nSubsidy >>= (nHeight / 4); // eGold: 4 blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // eGold: 1 hour
static const int64 nTargetSpacing = 6 * 60; // eGold: 6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eGold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eGold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x036128aa36257b2135a7a107afcc4b1b546b5c49ca693abe33095383059f21e3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "05/Giugno/2017 eGold, la nuova era dell'oro digitale";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c4507005111985") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1497569600;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1502136;
        ...
        assert(block.hashMerkleRoot == uint256("0xcfba466305e09333ac73900ab706e5b1eb649ea2f02e35a00a00bb71982b5b4e"));
	block.print();
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb2, 0xc3, 0xdb }; // eGold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eGoldMiner(CWallet *pwallet)
{
    printf("eGoldMiner started\n");
    ...
    try { loop {
        ...
        printf("Running eGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eGoldMiner, pwallet));
}
